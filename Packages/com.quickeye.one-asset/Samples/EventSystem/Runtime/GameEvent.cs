...
namespace QuickEye.EventSystem
{
    ...
    public abstract class GameEvent<TArgs> : GameEventBase, IInvokable
    {
        ...
        [field: NonSerialized]
        public UnityEvent<TArgs> Event { get; protected set; } = new UnityEvent<TArgs>();

        public TArgs LastPayload => _lastPayload;

        public virtual void Subscribe(UnityAction<TArgs> callback)
        {
            Event.AddListener(callback);
        }

        public virtual void Unsubscribe(UnityAction<TArgs> callback)
        {
